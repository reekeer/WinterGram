...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {
    ...
    private var skipWinterGramStickerConfirmation = false
    private var skipWinterGramGifConfirmation = false
    ...
    let winterGramPresenceTrackerDisposable = MetaDisposable()
    ...
    public init(
        ...
        let controllerInteraction = ChatControllerInteraction(openMessage: { [weak self] message, params in
            ...
            if !force && message.areReactionsTags(accountPeerId: strongSelf.context.account.peerId) {
                ...
                strongSelf.chatDisplayNode.historyNode.forEachItemNode { itemNode in
                    ...
                    switch reaction {
                    case .default:
                        let winterGramCustomReaction = currentWinterGramSettings.customDefaultReaction
                        if !winterGramCustomReaction.isEmpty {
                            // WinterGram: override the double-tap quick reaction with the chosen emoji.
                            chosenReaction = .builtin(winterGramCustomReaction)
                        } else {
        ...
        clickThroughMessage: { [weak self] view, location in
            ...
            if currentWinterGramSettings.stickerConfirmation && !strongSelf.skipWinterGramStickerConfirmation {
                strongSelf.present(textAlertController(
                    context: strongSelf.context,
                    updatedPresentationData: strongSelf.updatedPresentationData,
                    title: "Send Sticker?",
                    text: "WinterGram confirmation is enabled.",
                    actions: [
                        TextAlertAction(type: .genericAction, title: strongSelf.presentationData.strings.Common_Cancel, action: {}),
                        TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.strings.Common_OK, action: { [weak strongSelf] in
                            guard let strongSelf else {
                                return
                            }
                            strongSelf.skipWinterGramStickerConfirmation = true
                            ...
                            strongSelf.skipWinterGramStickerConfirmation = false
                        })
                    ]
                ), in: .window(.root))
                return true
            ...
            if let strongSelf = self {
                if currentWinterGramSettings.gifConfirmation && !strongSelf.skipWinterGramGifConfirmation {
                    strongSelf.present(textAlertController(
                        context: strongSelf.context,
                        updatedPresentationData: strongSelf.updatedPresentationData,
                        title: "Send GIF?",
        ...
        if let activitySpace = activitySpace {
            ...
            |> deliverOnMainQueue).startStrict(next: { [weak self] value in
                ...
                let s = currentWinterGramSettings
                if s.ghostModeEnabled && !s.sendUploadProgress { return }
                if strongSelf.presentationInterfaceState.interfaceState.editMessage == nil && strongSelf.presentationInterfaceState.subject != .scheduledMessages && strongSelf.presentationInterfaceState.currentSendAsPeerId == nil {
    ...
    deinit {
        ...
        self.winterGramPresenceTrackerDisposable.dispose()
        ...
        if !self.checkedPeerChatServiceActions {
            ...
            if case let .peer(peerId) = self.chatLocation, self.screenCaptureManager == nil {
                if peerId.namespace == Namespaces.Peer.SecretChat {
                    if !currentWinterGramSettings.allowScreenshots {
    ...
    func transformEnqueueMessages(_ messages: [EnqueueMessage], silentPosting: Bool, scheduleTime: Int32? = nil, repeatPeriod: Int32? = nil, postpone: Bool = false) -> [EnqueueMessage] {
        ...
        let effectiveSilentPosting: Bool
        switch settings.sendWithoutSound {
        case .never:
            effectiveSilentPosting = silentPosting
        case .inGhostMode:
            effectiveSilentPosting = silentPosting || settings.ghostModeEnabled
        case .always:
            effectiveSilentPosting = true
        }

        var scheduleTime = scheduleTime
        if scheduleTime == nil, settings.useScheduledMessages, case .peer = self.chatLocation, self.presentationInterfaceState.subject != .scheduledMessages {
            // "Отложка": opt-in only via the dedicated useScheduledMessages toggle (independent of
            // ghost mode). Routes the message through the scheduled-messages path with a near-immediate
            // time so sending does not mark the chat as read.
            scheduleTime = Int32(Date().timeIntervalSince1970) + 12
        }
    ...
    func sendMessages(_ messages: [EnqueueMessage], media: Bool = false, postpone: Bool = false, commit: Bool = false) {
        ...
        |> deliverOnMainQueue).startStandalone(next: { [weak self] shouldDivert in
            ...
            if commit || !isScheduledMessages {
                ...
                // WinterGram ghost toggles, applied after an explicit send action.
                let wntSettings = currentWinterGramSettings
                if wntSettings.shouldMarkReadAfterAction, let latestIndex = self.chatDisplayNode.historyNode.latestMessageInCurrentHistoryView()?.index {
                    // Reads are suppressed by ghost mode, but the user engaged: mark the chat read.
                    let _ = self.context.engine.messages.applyMaxReadIndexInteractively(index: latestIndex).startStandalone()
                }
                if wntSettings.shouldGoOfflineAfterAction {
                    let _ = self.context.engine.accountData.wntSendOfflinePresenceUpdate().startStandalone()
                }
