    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList, accountPeer -> ContextController.Items in
        ...
        if !messageText.isEmpty || richMessageMarkdown != nil || (resourceAvailable && isImage) || diceEmoji != nil {
            if !isExpired {
                ...
                if let editHistory = message.attributes.first(where: { $0 is WinterGramEditHistoryAttribute }) as? WinterGramEditHistoryAttribute, !editHistory.revisions.isEmpty {
                    actions.append(.action(ContextMenuActionItem(text: "Edit History (\(editHistory.revisions.count))", icon: { theme in
                        ...
                        c?.dismiss(completion: {
                            ...
                            items.append(ActionSheetTextItem(title: "Edit History"))
                            for revision in editHistory.revisions.reversed() {
                                let dateText = humanReadableStringForTimestamp(strings: presentationData.strings, dateTimeFormat: presentationData.dateTimeFormat, timestamp: revision.timestamp, alwaysShowTime: true, allowYesterday: true, format: nil).string
                                items.append(ActionSheetButtonItem(title: "\(dateText)\n\(revision.text)", action: { [weak sheet] in
                            ...
                            controllerInteraction.presentController(sheet, nil)
                        })
                        let _ = f
                    })))
                }
                ...
                let trimmedNumeric = message.text.trimmingCharacters(in: .whitespacesAndNewlines)
                if trimmedNumeric.count >= 5 && trimmedNumeric.count <= 16, let numericId = Int64(trimmedNumeric), numericId > 0 {
                    actions.append(.action(ContextMenuActionItem(text: "Open Profile #\(numericId)", icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/User"), color: theme.actionSheet.primaryTextColor)
                    }, action: { c, _ in
                        c?.dismiss(completion: {
                            let peerId = EnginePeer.Id(namespace: Namespaces.Peer.CloudUser, id: EnginePeer.Id.Id._internalFromInt64Value(numericId))
                            ...
                            |> deliverOnMainQueue).startStandalone(next: { peer in
                                if let peer = peer {
                                    ...
                                    let presentationData = context.sharedContext.currentPresentationData.with { $0 }
                                    controllerInteraction.presentController(textAlertController(context: context, title: "WinterGram", text: "No cached profile for ID \(numericId). You can only open users already known to the app.", actions: [TextAlertAction(type: .defaultAction, title: presentationData.strings.Common_OK, action: {})]), nil)
                                }
                            })
                        })
                    })))
                ...
                if !message.text.isEmpty && message.media.isEmpty {
                    actions.append(.action(ContextMenuActionItem(text: "Edit Locally", icon: { theme in
                        ...
                        c?.dismiss(completion: {
                            let controller = promptController(context: context, text: "Edit Locally", subtitle: "This change stays on your device only — it is not sent and adds no edited mark.", value: message.text, characterLimit: 4096, apply: { newValue in
                                if let newValue, !newValue.isEmpty {
                                    let _ = winterGramEditMessageLocally(postbox: context.account.postbox, messageId: message.id, text: newValue).startStandalone()
                                }
                            })
