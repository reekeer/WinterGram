...
func infoItems(
    ...
    if case let .user(user) = data.peer {
        ...
        // The WinterGram badge is now rendered next to the name (like the premium emoji status),
        // so the old "WinterGram Developer" profile row has been removed.

        if currentWinterGramSettings.showPeerId != .hidden {
            let rawId = user.id.id._internalGetInt64Value()
            // Data center is derived from the peer's profile-photo resource (CloudPeerPhotoSizeMediaResource).
            var winterGramDcId: Int?
            for representation in user.photo {
                if let resource = representation.resource as? CloudPeerPhotoSizeMediaResource {
                    winterGramDcId = resource.datacenterId
                    break
                }
            }
            var displayId = "\(rawId)"
        ...
        if currentWinterGramSettings.showPeerId != .hidden, !user.isDeleted, user.id != context.account.peerId {
            let isMutual = user.flags.contains(.mutualContact)
            let mutualText = isMutual ? presentationData.strings.WinterGram_MutualContact : presentationData.strings.WinterGram_NotMutualContact
            items[.winterGramInfo]!.append(PeerInfoScreenLabeledValueItem(id: 3095, label: "", text: mutualText, textColor: .primary, action: nil, requestLayout: { animated in
                interaction.requestLayout(animated)
            }))
        }
        ...
        if let _ = data.threadData {
            ...
            if currentWinterGramSettings.showPeerId != .hidden {
                let rawId = channel.id.id._internalGetInt64Value()
                let displayId: String
                switch currentWinterGramSettings.showPeerId {
                case .hidden, .telegramApi:
                    displayId = "\(rawId)"
                case .botApi:
                    displayId = "-100\(rawId)"
                }
                items[currentPeerInfoSection]!.append(PeerInfoScreenLabeledValueItem(id: 99, label: "ID", text: displayId, textColor: .primary, action: nil, longTapAction: nil, contextAction: { _, gesture, _ in
