...
final class PeerInfoHeaderNode: ASDisplayNode {
    ...
    let titleWinterGramIconView: ComponentHostView<Empty>
    var winterGramIconSize: CGSize?
    let titleExpandedWinterGramIconView: ComponentHostView<Empty>
    var titleExpandedWinterGramIconSize: CGSize?
    ...
    init(context: AccountContext, controller: PeerInfoScreenImpl, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, isMediaOnly: Bool, isSettings: Bool, isMyProfile: Bool, forumTopicThreadId: Int64?, chatLocation: ChatLocation) {
        ...
        self.titleWinterGramIconView = ComponentHostView<Empty>()
        self.titleNode.stateNode(forKey: TitleNodeStateRegular)?.view.addSubview(self.titleWinterGramIconView)

        self.titleExpandedWinterGramIconView = ComponentHostView<Empty>()
        self.titleNode.stateNode(forKey: TitleNodeStateExpanded)?.view.addSubview(self.titleExpandedWinterGramIconView)
    ...
    private var currentWinterGramIcon: Bool = false
    ...
    func update(width: CGFloat, containerHeight: CGFloat, containerInset: CGFloat, statusBarHeight: CGFloat, navigationHeight: CGFloat, isModalOverlay: Bool, isMediaOnly: Bool, contentOffset: CGFloat, paneContainerY: CGFloat, presentationData: PresentationData, peer: EnginePeer?, cachedData: EngineCachedPeerData?, threadData: MessageHistoryThreadData?, peerNotificationSettings: TelegramPeerNotificationSettings?, threadNotificationSettings: TelegramPeerNotificationSettings?, globalNotificationSettings: EngineGlobalNotificationSettings?, statusData: PeerInfoStatusData?, panelStatusData: (PeerInfoStatusData?, PeerInfoStatusData?, CGFloat?), isSecretChat: Bool, isContact: Bool, isSettings: Bool, state: PeerInfoState, profileGiftsContext: ProfileGiftsContext?, screenData: PeerInfoScreenData?, isSearching: Bool, metrics: LayoutMetrics, deviceMetrics: DeviceMetrics, transition: ContainedViewLayoutTransition, additive: Bool, animateHeader: Bool) -> CGFloat {
        ...
        let isWinterGramOfficial = peer.map { isWinterGramOfficialPeer($0) } ?? false
        ...
        if let peer {
            ...
            } else if (peer.isPremium || (currentWinterGramSettings.localPremium && (peer.id == self.context.account.peerId && (self.isSettings || self.isMyProfile)))) && !premiumConfiguration.isPremiumDisabled && (peer.id != self.context.account.peerId || self.isSettings || self.isMyProfile) {
        ...
        do {
            self.currentWinterGramIcon = isWinterGramOfficial

            let badgeContent: EmojiStatusComponent.Content = isWinterGramOfficial ? .winterGramBadge(backplateColor: winterGramBadgeBackplateColor(theme: presentationData.theme)) : EmojiStatusComponent.Content.none

            let iconSize = self.titleWinterGramIconView.update(
                transition: ComponentTransition(navigationTransition),
                component: AnyComponent(EmojiStatusComponent(
                    context: self.context,
                    animationCache: self.animationCache,
                    ...
                    useSharedAnimation: true,
                    action: { [weak self] in
                        self?.presentWinterGramBadgeInfo()
                    }
                )),
                environment: {},
                containerSize: CGSize(width: 26.0, height: 26.0)
            )
            let expandedIconSize = self.titleExpandedWinterGramIconView.update(
                transition: ComponentTransition(navigationTransition),
                component: AnyComponent(EmojiStatusComponent(
                    context: self.context,
                    animationCache: self.animationCache,
            ...
            self.winterGramIconSize = iconSize
            self.titleExpandedWinterGramIconSize = expandedIconSize
        }
        ...
        if let winterGramIconSize = self.winterGramIconSize, let titleExpandedWinterGramIconSize = self.titleExpandedWinterGramIconSize, winterGramIconSize.width > 0.0 {
            ...
            let winterGramLeadingGap: CGFloat = 10.0
            let offset = (winterGramIconSize.width + winterGramLeadingGap) / 2.0

            let leftOffset: CGFloat = nextIconX + winterGramLeadingGap
            let leftExpandedOffset: CGFloat = nextExpandedIconX + winterGramLeadingGap
            titleHorizontalOffset -= offset
            ...
            transition.updateFrame(view: self.titleWinterGramIconView, frame: CGRect(origin: CGPoint(x: leftOffset + collapsedTransitionOffset, y: floor((titleSize.height - winterGramIconSize.height) / 2.0)), size: winterGramIconSize))
            transition.updateFrame(view: self.titleExpandedWinterGramIconView, frame: CGRect(origin: CGPoint(x: leftExpandedOffset, y: floor((titleExpandedSize.height - titleExpandedWinterGramIconSize.height) / 2.0) + 1.0), size: titleExpandedWinterGramIconSize))

            nextIconX += winterGramLeadingGap + winterGramIconSize.width
            nextExpandedIconX += winterGramLeadingGap + titleExpandedWinterGramIconSize.width
        }
        ...
        if let profileGiftsContext, let peer {
            // WinterGram: visual gifts are listed in the Gifts pane only; they are NOT auto-pinned onto
            // the profile cover. (Pinning to the cover would be a manual per-gift action.)
            let additionalGifts: [ProfileGiftsContext.State.StarGift] = []
    ...
    // WinterGram: tapping the badge shows a supporter-style tooltip with a "Learn more" action,
    // mirroring the AyuGram/exteraGram unique-badge popup.
    private func presentWinterGramBadgeInfo() {
        ...
        if case .channel = peer {
            text = "\(name) \(strings.WinterGram_BadgeOfficialChannelSuffix)"
        } else if isWinterGramDeveloperPeer(peer) {
            // Actual developers are labelled as developers, not as supporters.
            text = "\(name) \(strings.WinterGram_BadgeDeveloperRole)"
        } else {
            text = "\(name) \(strings.WinterGram_BadgeDeveloperSuffix)"
        }
        let badgeImage = winterGramBadgeImage(for: peer, theme: presentationData.theme) ?? UIImage(bundleImageName: "WinterGramSnowflake")
        let content: UndoOverlayContent
        if let badgeImage {
            content = .universalImage(image: badgeImage, size: CGSize(width: 32.0, height: 32.0), title: nil, text: text, customUndoText: strings.WinterGram_LearnMore, timeout: nil)
        } else {
            content = .info(title: nil, text: text, timeout: nil, customUndoText: strings.WinterGram_LearnMore)
        }
        let overlayController = UndoOverlayController(presentationData: presentationData, content: content, elevatedLayout: false, action: { [weak self] action in
            if case .undo = action, let self {
                self.context.sharedContext.applicationBindings.openUrl("https://github.com/reekeer/WinterGram")
            }
            return true
        })
        self.controller?.present(overlayController, in: .current)
    }
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if !(self.state?.isEditing ?? false) {
            ...
            if self.currentWinterGramIcon {
                let iconFrame = self.titleWinterGramIconView.convert(self.titleWinterGramIconView.bounds, to: self.view)
                let expandedIconFrame = self.titleExpandedWinterGramIconView.convert(self.titleExpandedWinterGramIconView.bounds, to: self.view)
                if expandedIconFrame.contains(point) && self.isAvatarExpanded {
                    return self.titleExpandedWinterGramIconView.hitTest(self.view.convert(point, to: self.titleExpandedWinterGramIconView), with: event)
                } else if iconFrame.contains(point) {
                    return self.titleWinterGramIconView.hitTest(self.view.convert(point, to: self.titleWinterGramIconView), with: event)
                }
            }
