...
final class GiftsListView: UIView {
    ...
    init(context: AccountContext, peerId: EnginePeer.Id, profileGifts: ProfileGiftsContext, giftsCollections: ProfileGiftsCollectionsContext?, canSelect: Bool, ignoreCollection: Int32? = nil, remainingSelectionCount: Int32 = 0) {
        ...
        self.dataDisposable = combineLatest(
        ...
        ).startStrict(next: { [weak self] state, reorderedReferences, winterGramSettings in
            guard let self else {
                return
            }
            ...
            if self.peerId == self.context.account.peerId {
                for visualGift in winterGramSettings.visualGifts {
                    let gift = ProfileGiftsContext.State.StarGift(
                        gift: visualGift.gift,
                        reference: .slug(slug: "visual_\(visualGift.id)"),
                        fromPeer: visualGift.fromPeer,
                        date: 0,
                        text: nil,
                        entities: nil,
                        nameHidden: false,
                    ...
                    if !stateItems.contains(where: { $0.reference == gift.reference }) {
                        stateItems.append(gift)
                    }
                }
            }

            self.statusPromise.set(.single(PeerInfoStatusData(text: presentationData.strings.SharedMedia_GiftCount(Int32(stateItems.count)), isActivity: true, key: .gifts)))
            ...
            self.resultsAreEmpty = state.filter == .All && stateItems.isEmpty && state.dataState != .loading
            self.filteredResultsAreEmpty = state.filter != .All && stateItems.isEmpty
