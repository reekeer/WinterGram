        ...
        switch operation {
            ...
            case let .DeleteMessages(ids):
                ...
                if winterGramShouldSave && !currentWinterGramCoreSettings.saveForBots {
                    if let firstPeerId = ids.first?.peerId, let peer = transaction.getPeer(firstPeerId) as? TelegramUser, peer.botInfo != nil {
                        winterGramShouldSave = false
                    }
                }
                if winterGramShouldSave {
                    let markDate = Int32(Date().timeIntervalSince1970)
                    winterGramRecordDeletedMessages(transaction: transaction, ids: ids)
                    for id in ids {
            ...
            case let .EditMessage(id, message):
                ...
                transaction.updateMessage(id, update: { previousMessage in
                    ...
                    } else if currentWinterGramCoreSettings.saveMessageEditHistory {
                        var revisions = (previousMessage.attributes.first(where: { $0 is WinterGramEditHistoryAttribute }) as? WinterGramEditHistoryAttribute)?.revisions ?? []
                        revisions.append(WinterGramEditHistoryAttribute.Revision(
                            text: previousMessage.text,
                            entities: previousMessage.textEntitiesAttribute?.entities ?? [],
                            timestamp: previousMessage.timestamp
                        ))
                        updatedAttributes.removeAll(where: { $0 is WinterGramEditHistoryAttribute })
                        updatedAttributes.append(WinterGramEditHistoryAttribute(revisions: revisions))
                    }
